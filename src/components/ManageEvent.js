...
const ManageEvent = () => {
    ...
    const handleEdit = (eventId, eventName, eventDate, eventTime, eventDescription) => {
    ...
    const handleUpdate = async () => {
        try {
            const eventRef = ref(database, `simple_events/${editEventId}`);
            await set(eventRef, {
    ...
    const handleDelete = async (eventId) => {
        ...
        if (confirmDelete) {
            try {
                const eventRef = ref(database, `simple_events/${eventId}`);
    ...
    return (
        <div className="container flex-col mx-auto p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4 text-center">Manage Events</h2>
                ...
                {events.map((event) => (
                    <li key={event.id} className="border p-4 rounded-md shadow-md">
                        ...
                        {editEventId === event.id ? (
                            ...
                            <div className="flex space-x-4">
                                <button onClick={() => handleEdit(event.id, event.eventName, event.eventDate, event.eventTime, event.eventDescription)} className="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 transition duration-300 ease-in-out">Edit</button>
